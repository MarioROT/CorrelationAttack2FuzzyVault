...
Vaults= dir(strcat(pwd,'\Vaults\*.tx'));

Vaults= dir(strcat(pwd,'\Vaults\*.txt'));
...
%%
% distancias=linspace(6000,9000,5);
distancias = [2500];
...
for d = 1:length(distancias)
    ...
    for vu = 1:scn1
        for vc = 1:scn2
            ...
            VaultFij = load(strcat('Vaults\Vault',VaultFijName, '.txt'));  %%%% Vault Extraída de Sistema 1
            VaultMov = load(strcat('Vaults\Vault',VaultMovName, '.txt'));  %%%% Vault Extraída de Sistema 2



            Real_XY1 = load(strcat('Real_XYs\Real_XY', VaultFijName, '.txt'));  % Puntos genuinos VaultFij
            Real_XY2 = load(strcat('Real_XYs\Real_XY', VaultMovName, '.txt'));  % Puntos genuinos VaultMov
            Chaff_Data1 = load(strcat('Chaff_Datas\Chaff_Data', VaultFijName, '.txt'));  % Puntos de ruido VaultFij
            Chaff_Data2 = load(strcat('Chaff_Datas\Chaff_Data', VaultMovName,'.txt'));  % Puntos de ruido VaultMov
...
%             VaultMovRot = VaultMovRotf;
            ...
            if g_enc >=9
                ...
                combs_pos = factorial(g_enc)/(factorial(9)*factorial(g_enc-9));
                if combs_pos < min_combs
                    combs_ft(m_n,:) = [VaultFijName, VaultMovName, combs_pos]; 
                end
            end
...
% save('ResCorr10_6000_9000mindistSoloGiro.mat','resultados')
save('CombsPos_mindist2500_SoloGiro.mat','combs_ft')
...
save('TabResCorr10_6000_9000mindistSoloGiro.mat','TabRes')
%%
